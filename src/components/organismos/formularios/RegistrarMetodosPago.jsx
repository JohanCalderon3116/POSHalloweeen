...
/* ========================= ANIMACIONES ========================= */ const telaranaAnim = keyframes` 0%, 100% { opacity: 0.025; transform: rotate(0deg) scale(1); } 50% { opacity: 0.055; transform: rotate(1.5deg) scale(1.015); } `;
const murcielagoVolar = keyframes` 0% { transform: translateX(-120px) translateY(0) rotate(-3deg); } 20% { transform: translateX(20vw) translateY(-8px) rotate(3deg); } 40% { transform: translateX(45vw) translateY(6px) rotate(-2deg); } 60% { transform: translateX(70vw) translateY(-6px) rotate(2deg); } 80% { transform: translateX(90vw) translateY(4px) rotate(-2deg); } 100% { transform: translateX(calc(100vw + 120px)) translateY(0) rotate(2deg); } `;
const aletear = keyframes` 0%, 100% { transform: scaleY(1); } 50% { transform: scaleY(0.55); } `;
const arañaCaer = keyframes` from { transform: translateY(-45px); opacity: 0; } to { transform: translateY(0); opacity: 1; } `;
const arañaBalancear = keyframes` 0%, 100% { transform: rotate(-5deg); } 50% { transform: rotate(5deg); } `;
const aparecer = keyframes` from { opacity: 0; transform: translateY(10px) scale(0.985); } to { opacity: 1; transform: translateY(0) scale(1); } `;
/* ========================= TELARAÑA ========================= */ const telaranaPaths = ` M0 0 L180 0 M0 0 L165 70 M0 0 L125 125 M0 0 L70 165 M0 0 L0 180 M30 0 Q28 18 0 30 M60 0 Q58 34 0 60 M90 0 Q86 50 0 90 M120 0 Q114 68 0 120 M150 0 Q144 85 0 150 `;
...
/* ========================= MURCIÉLAGO ========================= */ const MurcielagoSvg =
  () => (
...
/* ========================= ARAÑA ========================= */ const AranaSvg =
  () => (
    <svg viewBox="0 0 60 60" width="100%" height="100%" aria-hidden="true">
      ...
      >
        {" "}
        <path d="M36 24 L49 11 L56 19" /> <path d="M37 28 L53 21 L58 31" />{" "}
        <path d="M37 32 L53 35 L57 47" /> <path d="M35 36 L47 45 L49 55" />{" "}
        <path d="M24 24 L11 11 L4 19" /> <path d="M23 28 L7 21 L2 31" />{" "}
        <path d="M23 32 L7 35 L3 47" /> <path d="M25 36 L13 45 L11 55" />{" "}
      </g>{" "}
      <ellipse
        cx="30"
        cy="38"
...
/* ========================= COMPONENTE ========================= */ export function RegistrarMetodosPago({
  onClose,
  dataSelect,
  accion,
  setIsExploding,
    ...
    <Container>
      ...
      {isPending ? (
        <ConteinerLoader>
          ...
          <BeatLoader
            color={theme.halloweenPrimary || theme.text}
            size={8}
          />{" "}
        </ConteinerLoader>
      ) : (
...
/* ========================= CONTAINER PRINCIPAL ========================= */ const Container = styled.div`
  position: fixed;
  inset: 0;
...
`;
/* ========================= DECORACIÓN ========================= */ const Decoracion = styled.div`
  position: fixed;
  inset: 0;
  pointer-events: none;
  overflow: hidden;
...
`;
/* ========================= TELARAÑAS ========================= */ const WebCorner = styled.div`
  position: absolute;
  top: -12px;
  width: 180px;
  height: 180px;
...
`;
/* ========================= MURCIÉLAGO ========================= */ const Bat = styled.div`
  position: absolute;
  top: 14%;
  left: -120px;
  width: 48px;
...
const BatInner = styled.div`
  ...
  will-change: transform;
`;
/* ========================= ARAÑAS ========================= */ const Spider = styled.div`
  position: absolute;
  top: 0;
...
`;
/* ========================= IMAGEN ========================= */ const PictureContainer = styled.div`
  display: flex;
  align-items: center;
  justify-content: start;
  border: 2px dashed ${({ theme }) => theme.halloweenPrimary || "#b85c18"};
  border-radius: 8px;
  background: rgba(249, 215, 11, 0.06);
  padding: 8px;
  position: relative;
  gap: 8px;
  margin-bottom: 15px;
...
`;
/* ========================= LOADER ========================= */ const ConteinerLoader = styled.div`
  display: flex;
  align-items: center;
  justify-content: center;
  flex-direction: column;
